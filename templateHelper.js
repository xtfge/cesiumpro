 ...
 */
 ...
 function buildLink(longname, linkText, options) {
     ...
     if ( hasUrlPrefix(stripped) ) {
         ...
         fileUrl = `../cesiumDocumentation/${upath[0].replace('Cesium.', '')}.html${hash}`;
         text = linkText || stripped;
     ...
     if (!fileUrl) {
         ...
         return util.format('<a href="%s"%s%s>%s</a>', encodeURI(fileUrl + fragmentString),
             classString, target, text);
     }
 }
