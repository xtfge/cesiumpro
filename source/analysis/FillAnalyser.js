...
function getNodesAndHeight(viewer, points, exclude = []) {
    const positions = [];
    let h = 0;
    const area = AnalyserUtil.getArea(points);
    for (let i = 0; i < 3; i++) {
...
class FillAnalyser extends BaseAnalyser {
    /**
     * 挖方分析
    ...
    constructor(viewer, options = {}) {
        ...
        // }
        const mask = AnalyserUtil.getCartesians(options.mask);
        if (mask) {
    ...
    setMask(mask) {
        ...
        const maskCartesians = AnalyserUtil.getCartesians(options.mask);        
    ...
    do() {
        ...
        const grid = AnalyserUtil.triangleGrid(polygon, cellSize);
