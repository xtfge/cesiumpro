...
class BaseGraphic {
     ...
     */
    get properties() {
        const props = { id: this.gvid, type: this.gvtype, name: this.gvname, description: this.description }

        return { ...props, ...this._properties }
    }
    set properties(v) {
...
class CesiumPolygon extends BaseGraphic {
     ...
     * @param {Object} properties 属性信息
     */
    static fromRadians(viewer, positions,properties = {}) {
        positions = positions.map(_ => {
            return Cesium.Cartesian3.fromRadians(_.lon, _.lat, _.height);
        });
        const options = CesiumPolygon.defaultStyle;
