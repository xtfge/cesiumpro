...
class CesiumProTileAvailability extends TileAvailability {
    constructor(tilingScheme, maximumLevel) {
        super(tilingScheme, maximumLevel);
    ...
    addAvailableTileRange(
        ...
        const key = this.getKey(level, startX, startY, endX, endY);
        if (this.cached.includes(key)) {
            return;
        }
        this.rangeChange.raise(this, level, startX, startY, endX, endY);
        this.cached.push(key)
        super.addAvailableTileRange(level, startX, startY, endX, endY);
    };
    getKey(level, startX, startY, endX, endY) {
        return 'key' + level + startX + startY + endX + endY;
    }
}
...
function requestTileGeometry(provider, x, y, level, layerToUse, request) {
    ...
    if (source.request.state === RequestState.ISSUED ||
        source.request.state === RequestState.ACTIVE) {
        return;
    }
...
export default CesiumProTerrainProvider;
