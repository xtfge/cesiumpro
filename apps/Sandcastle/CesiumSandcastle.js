...
require({
    ...
    function activateBucketScripts(bucketDoc) {
        ...
        var loadScript = function() {
            ...
            if (nodes.length > 0) {
                ...
                if(bucketFrame.contentWindow.CesiumPro&&bucketFrame.contentWindow.Cesium){
    ...
    function createGalleryButton(demo, tabName) {
        ...
        demoLink.onclick = function(e) {
            if (mouse.isMiddle(e)) {
                ...
                if (confirmChange) {
                    ...
                    window.history.pushState(demo, demo.title, getPushStateUrl(demo));
                    loadFromGallery(demo).then(function() {
                        document.title = demo.name + ' - Cesium Sandcastle';
                    });
                }
        ...
        new LinkButton({
            'label' : '<div class="demoTileTitle">' + demo.title + '</div>' +
